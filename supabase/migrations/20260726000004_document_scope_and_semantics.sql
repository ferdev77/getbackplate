-- A direct user assignment grants access. Values inside one dimension are OR,
-- while the populated location, department, and position dimensions are AND.
...
begin
  ...
  -- A user-only scope remains private when the current user is not listed.
  if v_location_count = 0 and v_department_count = 0 and v_position_count = 0
...
begin
  ...
  from public.resolve_folder_effective_scope(folder_org_id, folder_id) resolved;
...
begin
  ...
  if not v_has_explicit_scope and doc_folder_id is not null then
    select resolved.scope
    into v_effective_scope
    from public.resolve_folder_effective_scope(doc_org_id, doc_folder_id) resolved;
  end if;
...
revoke all on function public.current_user_matches_document_scope(uuid, uuid, jsonb, uuid) from public, anon;
grant execute on function public.current_user_matches_document_scope(uuid, uuid, jsonb, uuid) to authenticated, service_role;
