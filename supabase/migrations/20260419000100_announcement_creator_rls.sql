...
begin
  ...
  if ann_branch_id is not null and (v_effective_branch_id is null or ann_branch_id <> v_effective_branch_id) then
    return false;
  end if;
      ...
      where aa.organization_id = ann_org_id
        ...
        and (
          ...
          or (
            ...
            and v_effective_branch_id is not null
            and aa.branch_id = v_effective_branch_id
          )
        )
    );
  end if;
...
create policy announcements_tenant_select
  ...
  using (public.can_read_announcement(organization_id, id, branch_id, target_scope, created_by));
