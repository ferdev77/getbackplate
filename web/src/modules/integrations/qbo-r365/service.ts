...
import {
  buildQboR365DedupeKey as buildDedupeKey,
  buildQboR365FileName as buildFileName,
  getQboDisconnectRetryPlan,
  normalizeQboRows,
  type QboR365Mapping,
} from "@/modules/integrations/qbo-r365/pipeline-rules";
import {
...
type MappingRow = QboR365Mapping;
...
export async function listSyncConfigs(organizationId: string): Promise<SyncConfigSummary[]> {
  ...
  const { data, error } = await admin
    .from("qbo_r365_sync_configs")
    .select("id, name, qbo_customer_id, qbo_customer_name, schedule_interval, lookback_hours, template, tax_mode, status, last_run_at, r365_ftp_host, r365_ftp_username, r365_ftp_remote_path, r365_vendor_name, r365_location, created_at")
    .eq("organization_id", organizationId)
    .order("created_at", { ascending: true });
  ...
  return (data ?? []).map((row) => {
    ...
    return {
      ...
      r365FtpHost: (row.r365_ftp_host as string | null) ?? null,
      r365FtpUsername: (row.r365_ftp_username as string | null) ?? null,
      r365FtpRemotePath: (row.r365_ftp_remote_path as string | null) ?? null,
...
async function scheduleQboDisconnectRetry(connection: ConnectionRow, errorCode: string) {
  ...
  const retryPlan = getQboDisconnectRetryPlan(
    connection.qbo_disconnect_attempts,
    isApp ? "app_pending" : "portal_pending",
  );
  const { attempts, retryAt, reviewRequired: review, state } = retryPlan;
  const { data, error } = await admin.from("integration_connections").update({
