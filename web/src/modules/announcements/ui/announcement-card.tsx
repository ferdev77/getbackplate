...
export function AnnouncementCard({
  ...
  const publishLabel = announcement.publish_at ? new Date(announcement.publish_at).toLocaleDateString("es-MX") : "-";
    ...
    ? (() => {
        ...
        return (
          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 ${badgeClass}`}>
            <CalendarClock className="h-3 w-3" /> {prefix}: {new Date(announcement.expires_at).toLocaleDateString("es-MX")}
          </span>
        );
      })()
    : null;
