...
export function UploadDocumentModal({
  ...
  return (
    <div className={`fixed inset-0 z-[1000] flex items-center justify-center bg-black/45 p-5 transition-opacity duration-300 ${isClosing ? "opacity-0" : "opacity-100"}`}>
      <div className={`relative max-h-[90vh] w-[980px] max-w-[96vw] overflow-hidden rounded-2xl bg-white shadow-[0_24px_70px_rgba(0,0,0,.18)] transition duration-300 ${isClosing ? "scale-[0.985] opacity-0" : "scale-100 opacity-100"}`}>
        <div className="flex items-center justify-between border-b-[1.5px] border-[#f0f0f0] px-6 py-5"><p className="font-serif text-[15px] font-bold text-[#111]">Subir Archivo</p><Link href="/app/documents" className="grid h-8 w-8 place-items-center rounded-md text-[#bbb] hover:bg-[#f5f5f5] hover:text-[#111]">✕</Link></div>
        <form onSubmit={handleSubmit}>
          <div className="max-h-[68vh] overflow-y-auto px-6 py-5">
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
              <div>
                <label className="mb-4 block cursor-pointer rounded-2xl border-2 border-dashed border-[#eadfd8] bg-[#fdf9f7] px-5 py-8 text-center transition hover:border-[#d8c8be]">
                  <p className="text-3xl">📂</p>
                  <p className="mt-2 text-[15px] font-bold text-[#2b2420]">Arrastra tu archivo aquí</p>
                  <p className="mt-1 text-xs text-[#8b817c]">o haz clic para seleccionar desde tu computadora</p>
                  <span className="mt-4 inline-flex rounded-lg bg-[#111] px-3 py-2 text-xs font-bold text-white">Seleccionar archivo</span>
                  <p className="mt-3 text-[11px] text-[#b1a7a2]">PDF, DOCX, XLSX · Máx. 10 MB</p>
                  {selectedFileName ? <p className="mt-2 truncate text-xs font-semibold text-[#4a433f]">Archivo: {selectedFileName}</p> : null}
                  <input name="file" type="file" required className="sr-only" onChange={(event) => setSelectedFileName(event.target.files?.[0]?.name ?? "")} />
                </label>

                <label className="mb-1 block text-[11px] font-bold uppercase tracking-[0.1em] text-[#aaa]">Titulo (opcional)</label>
                ...
                <textarea className="mb-2 h-24 w-full resize-none rounded-lg border border-[#e8e8e8] px-3 py-2 text-sm" placeholder="Describe brevemente el contenido del documento..." />

                {isUploading ? (
                  <div className="mt-3 rounded-xl border border-[#e7ddd8] bg-[#fffaf8] p-3">
                    <div className="mb-2 flex items-center justify-between text-xs text-[#6d645f]"><span>Subiendo archivo...</span><span className="font-semibold">{progress}%</span></div>
              ...
              <aside className="rounded-xl border border-[#ece3de] bg-[#fffcfb] p-4">
                <p className="mb-3 text-[12px] font-bold text-[#433b36]">Subidos recientemente</p>
                <div className="space-y-2.5">
                  {recentDocuments.slice(0, 4).map((document) => {
                    ...
                    return (
                      <div key={document.id} className="rounded-lg border border-[#efe7e2] bg-white px-3 py-2">
                        <p className="truncate text-[12px] font-semibold text-[#2f2925]">{document.title}</p>
                        <p className="mt-0.5 text-[11px] text-[#978d88]">{branchName} · {new Date(document.created_at).toLocaleDateString("es-AR")}</p>
                        <p className="mt-1 text-[11px] font-semibold text-[#2d8f4f]">Activo</p>
                      </div>
                    );
                  })}
                  {!recentDocuments.length ? <p className="text-xs text-[#9b908b]">Sin cargas recientes.</p> : null}
                </div>
              </aside>
            </div>
          </div>
