...
function SubmitButton({ t }: { t: (s: string) => string }) {
  ...
  return (
    ...
    >
      {pending ? t("Guardando...") : t("Guardar puesto")}
    </button>
  );
}
...
export function InlinePositionForm({
  ...
  if (!open) {
    return (
      ...
      >
        <Plus className="h-3.5 w-3.5" /> {t("Agregar puesto")}
      </button>
    );
  }
  ...
  return (
    <div className="mt-3 w-full animate-in overflow-hidden rounded-xl border border-[var(--gbp-border)] bg-[var(--gbp-bg)] p-3 shadow-sm fade-in slide-in-from-top-2">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-xs font-bold text-[var(--gbp-text)]">{t("Nuevo puesto en")} {departmentName}</p>
        <button
      ...
      >
        ...
        <input name="name" required placeholder={t("Ej: Analista")} className="rounded-lg border border-[var(--gbp-border2)] bg-[var(--gbp-surface)] px-3 py-2 text-sm text-[var(--gbp-text)] outline-none transition-all placeholder:text-[var(--gbp-muted)] focus:border-[var(--gbp-accent)] focus:ring-1 focus:ring-[var(--gbp-accent)]" />
        <input name="description" placeholder={t("Descripción (opcional)")} className="rounded-lg border border-[var(--gbp-border2)] bg-[var(--gbp-surface)] px-3 py-2 text-sm text-[var(--gbp-text)] outline-none transition-all placeholder:text-[var(--gbp-muted)] focus:border-[var(--gbp-accent)] focus:ring-1 focus:ring-[var(--gbp-accent)]" />
