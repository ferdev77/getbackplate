...
function SubmitButton({ t }: { t: (s: string) => string }) {
  ...
  return (
    ...
    >
      {pending ? t("Guardando...") : t("Guardar departamento")}
    </button>
  );
}

export function InlineDepartmentForm({
  locale,
  createAction,
}: {
  locale?: "es" | "en";
  ...
  return (
    <div className="mt-3 mb-4 w-full animate-in overflow-hidden rounded-xl border border-[var(--gbp-border)] bg-[var(--gbp-bg)] p-4 shadow-sm fade-in slide-in-from-top-2">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-bold text-[var(--gbp-text)]">{t("Nuevo Departamento")}</p>
        <button
      ...
      >
        <input name="name" required placeholder={t("Nombre de departamento")} className="rounded-lg border border-[var(--gbp-border2)] bg-[var(--gbp-surface)] px-3 py-2 text-sm text-[var(--gbp-text)] outline-none transition-all placeholder:text-[var(--gbp-muted)] focus:border-[var(--gbp-accent)] focus:ring-1 focus:ring-[var(--gbp-accent)]" />
