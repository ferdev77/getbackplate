...
export function IntegrationVendorProfileSettingsCard({ initialProfile }: { initialProfile: Partial<VendorProfile> | null }) {
  ...
  return <article className="rounded-2xl border border-[var(--gbp-border)] bg-[var(--gbp-surface)] p-5">
    <div className="mb-3 flex items-start justify-between gap-3"><div><p className="inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-[0.1em] text-[var(--gbp-text2)]"><Building2 className="h-3.5 w-3.5" /> QuickBooks vendor profile</p><p className="mt-1 text-sm text-[var(--gbp-text2)]">Used as the vendor identity for invoices delivered to R365.</p></div><button type="button" onClick={() => editing ? void save() : setEditing(true)} disabled={saving} className="rounded-lg bg-[var(--gbp-accent)] px-3 py-2 text-xs font-bold text-white disabled:opacity-60">{saving ? "Saving..." : editing ? "Save" : "Edit"}</button></div>
