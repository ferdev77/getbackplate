    ...
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-semibold text-slate-800">Email
          <input name="email" required type="email" maxLength={254} autoComplete="email" className={inputClass} />
        </label>
        <label className="text-sm font-semibold text-slate-800">Company <span className="font-normal text-slate-500">(optional)</span>
          <input name="company" maxLength={160} autoComplete="organization" className={inputClass} />
        </label>
      </div>
      ...
      <button type="submit" disabled={status.kind === "sending"} className="mt-5 rounded-lg bg-[#d4531a] px-5 py-3 text-sm font-bold text-white hover:bg-[#b94313] disabled:opacity-60">
