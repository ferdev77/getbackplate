...
export async function PUT(request: Request) {
  ...
  if (!(await isModuleEnabledForOrganization(access.tenant.organizationId, "qbo_r365"))) {
    return NextResponse.json({ error: "La integración QuickBooks no está activa." }, { status: 403 });
  }

  const parsed = vendorProfileSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "Datos de vendor inválidos." }, { status: 400 });
  ...
  if (error) return NextResponse.json({ error: "No se pudo guardar el perfil de vendor." }, { status: 500 });
