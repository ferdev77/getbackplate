...
import { deleteMaintenanceServiceItem, updateMaintenanceServiceItem } from "@/modules/maintenance/services";
...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const serviceItem = await updateMaintenanceServiceItem({
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  ...
  try {
    const { id } = await params;
    await deleteMaintenanceServiceItem({
      organizationId: access.tenant.organizationId,
      userId: access.userId,
      branchId: access.tenant.branchId,
      roleCode: access.tenant.roleCode,
