...
/**
 * Lo mismo que en la ruta de company, pero desde el portal del empleado: son
 * dos handlers espejo y el gate del borrador vive en cada uno por separado.
 */
...
function request(campos: Record<string, string>) {
  ...
  return new Request("https://test.invalid/api/employee/maintenance", { method: "POST", body: formData });
}
...
describe("POST /api/employee/maintenance", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.access.mockResolvedValue({
      ok: true,
      userId: USER_ID,
      tenant: { organizationId: ORG_ID, branchId: BRANCH_ID, roleCode: "employee" },
    });
